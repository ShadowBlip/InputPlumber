...
impl Driver {
    pub fn new(
        ...
        Ok(Self {
            ...
            accel_state: Default::default(),
            gyro_state: Default::default(),
        })
    }
    ...
    fn build_group(
        ...
        let sample_rate = Duration::from_secs_f64(1.0 / x.info.sample_rate);

        log::debug!("Sample rate in secs: {sample_rate:?}");
    ...
    fn is_buffer_empty(err: &industrial_io::Error) -> bool {
        ...
        matches!(
            err,
            industrial_io::Error::Nix(errno)
                if matches!(
                    *errno as i32,
                    x if x == nix::errno::Errno::EAGAIN as i32
                        || x == nix::errno::Errno::EBUSY as i32
                )
        )
    }
    ...
    pub fn poll(&mut self) -> Result<Vec<Event>, Box<dyn Error + Send + Sync>> {
        ...
        match refill_res {
            ...
            Err(e) if Self::is_buffer_empty(&e) => {
                log::debug!("Buffer has no data. Forcing read to fill buffer");

                if let Some(accel) = &self.accel {
                    accel.x.channel.attr_read_int("raw")?;
                }

                if let Some(gyro) = &self.gyro {
                    gyro.x.channel.attr_read_int("raw")?;
                }
                ...
                match self.buffer.refill() {
                    ...
                    Err(e) if Self::is_buffer_empty(&e) => {
                        log::debug!("Buffer has no data after retry.");
                        return Ok(events);
                    }
    ...
    fn poll_accel(&mut self) -> Result<Vec<Event>, Box<dyn Error + Send + Sync>> {
        ...
        while let (Some(x), Some(y), Some(z)) = (x_iter.next(), y_iter.next(), z_iter.next()) {
            ...
            // AMD_SFH firwmwares have an off-by-one error that causes the scale calculation to be
            // off by a factor of 10. Fix that here.
            // TODO: Add a configuration value to determine if this correction needs to be applied.
            let mut out = AxisData {
                roll: (*x as f64 + accel.x.info.offset as f64) * accel.x.info.scale * 10.0,
                pitch: (*y as f64 + accel.y.info.offset as f64) * accel.y.info.scale * 10.0,
                yaw: (*z as f64 + accel.z.info.offset as f64) * accel.z.info.scale * 10.0,
            };
            self.rotate_value(&mut out);

            // Ignore events with no changes
            let Some(accel_state) = &self.accel_state else {
                log::debug!("No accel_state");
                self.accel_state = Some(out);
                return Ok(events);
            };
            if *accel_state != out {
                events.push(Event::Accelerometer(out.clone()));
                self.accel_state = Some(out);
            }
        }
        ...
        if let Some(remaining) = accel.sample_rate.checked_sub(poll_time) {
            log::trace!("Sleep time remaining: {remaining:?}");
            thread::sleep(remaining);
        } else {
            log::trace!("Total poll time: {poll_time:?} ");
        }
        Ok(events)
    }

    /// Polls all the channels from the gyro and drains the buffer
    fn poll_gyro(&mut self) -> Result<Vec<Event>, Box<dyn Error + Send + Sync>> {
        ...
        while let (Some(x), Some(y), Some(z)) = (x_iter.next(), y_iter.next(), z_iter.next()) {
            ...
            // TODO: Add a configuration value to determine if this correction needs to be applied.
            let mut out = AxisData {
                roll: (*x as f64 + gyro.x.info.offset as f64) * gyro.x.info.scale * 10.0,
                pitch: (*y as f64 + gyro.y.info.offset as f64) * gyro.y.info.scale * 10.0,
                yaw: (*z as f64 + gyro.z.info.offset as f64) * gyro.z.info.scale * 10.0,
            };
            self.rotate_value(&mut out);

            // Ignore events with no changes
            let Some(gyro_state) = &self.gyro_state else {
                log::debug!("No gyro_state");
                self.gyro_state = Some(out);
                return Ok(events);
            };
            if *gyro_state != out {
                events.push(Event::Gyro(out.clone()));
                self.gyro_state = Some(out);
            }
        }
        ...
        let poll_time = poll_time_start.elapsed();
        // Sleep up to the configured duration after polling timeout
        if let Some(remaining) = gyro.sample_rate.checked_sub(poll_time) {
            log::trace!("Sleep time remaining: {remaining:?}");
            thread::sleep(remaining);
        } else {
            log::trace!("Total poll time: {poll_time:?} ");
