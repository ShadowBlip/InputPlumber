...
impl Driver {
    ...
    pub fn sample_delay_micro(&self) -> Result<u64, Box<dyn Error>> {
        return Ok((1.0 / self.get_sample_rate("gyro").unwrap_or(1.0) * 1000000.0) as u64);
    }
    ...
    /// Returns the available scales for the given measurement
    ...
    pub fn set_sample_rate(&mut self, imu_type: &str, rate: f64) -> Result<(), Box<dyn Error>> {
        match imu_type {
            ...
            _ => {
                // TODO: Why does this get mad at me?
                //Err(format!("{imu_type} is not a valid imu type.").into()),
                log::error!("{imu_type} is not a valid IMU type.")
            }
        }
        Ok(())
    }
    ...
    pub fn get_scale(&self, imu_type: &str) -> Result<f64, Box<dyn Error>> {
        ...
        match imu_type {
            "accel" => {
                let Some(info) = self.accel_info.get("accel_x") else {
                    return Err(format!(
                        "Unable to get sample rates available for IMU type {imu_type}"
                    )
                    .into());
                };
            ...
            "accel" => {
                for (id, channel) in &self.accel {
                    ...
                    if !info.scales_avail.contains(&scale) {
                        return Err(
                            format!("Unable to set scale to {scale}, scale not supported").into(),
                        );
                    }

                    let result = channel.attr_write_float("sampling_frequency", scale);
                    match result {
                        Ok(_) => info.scale = scale,
