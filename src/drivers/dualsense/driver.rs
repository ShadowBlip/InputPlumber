...
use super::{
    ...
    DS5_ACCEL_RAW_TO_MPS2, DS5_GYRO_RAW_TO_RAD_S, DS5_VID, INPUT_REPORT_BT_SIZE, PIDS,
};
...
impl Driver {
    ...
    fn translate(&mut self, old_state: Option<PackedInputDataReport>) -> Vec<Event> {
        ...
        events.push(Event::Accelerometer(IntertialEvent::Accelerometer(
            InertialInput {
                x: state.accel_x.to_primitive() as f64 * DS5_ACCEL_RAW_TO_MPS2,
                y: state.accel_y.to_primitive() as f64 * DS5_ACCEL_RAW_TO_MPS2,
                z: state.accel_z.to_primitive() as f64 * DS5_ACCEL_RAW_TO_MPS2,
            },
        )));
        events.push(Event::Accelerometer(IntertialEvent::Gyroscope(
            InertialInput {
                x: state.pitch.to_primitive() as f64 * DS5_GYRO_RAW_TO_RAD_S,
                y: state.yaw.to_primitive() as f64 * DS5_GYRO_RAW_TO_RAD_S,
                z: state.roll.to_primitive() as f64 * DS5_GYRO_RAW_TO_RAD_S,
            },
        )));
