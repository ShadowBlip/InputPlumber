...
fn convert_actuation_to_unit(value: f64) -> u8 {
    if !(KEY_TOP_MM..=KEY_BOTTOM_MM).contains(&value) {
        // 0 is allowed, but other values outside the key travel range are flagged as errors.
        if value != 0.0 {
            log::error!(
            ...
            ENDPOINT_2 => {
                if bytes_read == LONG_DATA_PAYLOAD {
                    return match buf[0] {
                        ...
                        0x1 => self.handle_basic(buf, KeyCodes::PhantomBlank, true),
                        0x6 => self.handle_analog(&buf[1..21]),
                        _ => Ok(Vec::new()),
                    };
                }
                Ok(Vec::new())
