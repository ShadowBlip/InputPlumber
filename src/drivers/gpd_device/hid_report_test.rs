...
fn test_parse_touch_report() {
    ...
    let buf: [u8; 29] = [
        0x04, 0x03, 0x12, 0x08, 0xa8, 0x02, 0x07, 0xf8, 0x03, 0xb9, 0x03, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xe8, 0xfe, 0x02, 0x03, 0x00, 0x00, 0x40, 0x4c,
    ];
    let report = TouchpadDataReport2024::unpack(&buf).unwrap();

    assert_eq!(report.report_id, 0x04);
    assert!(report.confidence0);
    assert!(report.tip_switch0);
    assert_eq!(report.contact_id0, 0);
    assert_eq!(report.touch_x0, 2066);
    assert_eq!(report.touch_y0, 680);

    assert!(report.confidence1);
    assert!(report.tip_switch1);
    ...
    let buf: [u8; 29] = [
        0x04, 0x01, 0x32, 0x08, 0xa6, 0x02, 0x05, 0xf5, 0x03, 0xc2, 0x03, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x68, 0x01, 0x02, 0x04, 0x00, 0x00, 0x40, 0x41,
    ];
    let report = TouchpadDataReport2024::unpack(&buf).unwrap();
    assert!(!report.tip_switch0);
    assert!(report.confidence0);
