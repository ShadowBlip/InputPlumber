...
// Report ID
const TOUCH_DATA: u8 = 0x04;

// Input report size
const TOUCHPAD_PACKET_SIZE: usize = 29;
...
pub struct TouchpadDriver2024 {
    ...
    touchpad_state: Option<TouchpadDataReport2024>,
}

impl TouchpadDriver2024 {
    pub fn new(udevice: UdevDevice) -> Result<Self, Box<dyn Error + Send + Sync>> {
        ...
        Ok(Self {
            ...
            first_touch_x: Default::default(),
            first_touch_y: Default::default(),
            is_clicked: Default::default(),
            is_touching: Default::default(),
            last_touch: Instant::now(),
            last_release: Instant::now(),
            touch_started: Default::default(),
            touchpad_state: Default::default(),
        })
    }
    ...
    pub fn poll(&mut self) -> Result<Vec<Event>, Box<dyn Error + Send + Sync>> {
        ...
        let mut events = match report_id {
            TOUCH_DATA => {
                ...
                let sized_buf: &[u8; TOUCHPAD_PACKET_SIZE] = slice.try_into()?;
                let input_report = TouchpadDataReport2024::unpack(sized_buf)?;
                let old_state = self.touchpad_state.replace(input_report);
                self.translate_touch(old_state)
            }
            _ => vec![],
        };
    ...
    fn translate_touch(&mut self, old_state: Option<TouchpadDataReport2024>) -> Vec<Event> {
