...
use crate::{
    ...
    drivers::grand_unified_controller::{
        capability::InputCapability,
        driver::{UNIFIED_CONTROLLER_PID, UNIFIED_CONTROLLER_VID},
        hid_report::{
            InputCapabilityInfo, InputCapabilityReport, InputDataReport, ReportType, ValueType,
        },
        report_descriptor::REPORT_DESCRIPTOR,
        update::{
            BoolUpdate, Int16Vector3Update, StateUpdate, UInt16Vector2Update, UInt8Update,
            ValueUpdate,
        },
        value::TouchValue,
    },
...
impl UnifiedGamepadDevice {
    ...
    fn update_capabilities(&mut self, capabilities: HashSet<Capability>) {
        ...
        let Some(composite_device) = self.composite_device.as_ref() else {
            log::error!("No composite device set to update capabilities");
            return;
        };
        ...
        let mut cap_info: Vec<InputCapabilityInfo> = capabilities
            ...
            .map(|cap| cap.into())
            .collect();
        cap_info.sort_by_key(|cap| cap.value_type);

        // Update the capability report
        self.capability_report = InputCapabilityReport::default();
        for info in cap_info {
            log::error!("Updating report with info: {info}");
            self.capability_report.add_capability(info);
        }
        ...
        if let Some(dbus_path) = self.dbus_path.as_ref() {
            ...
            {
                log::error!("Failed to update target capabilities: {e:?}");
            }
        }

        log::error!("Updated capabilities");
    }
...
impl TargetInputDevice for UnifiedGamepadDevice {
    ...
    fn start_dbus_interface(&mut self, dbus: Connection, path: String, client: TargetDeviceClient) {
        ...
        tokio::task::spawn(async move {
            let name = "Unified Gamepad".to_string();
    ...
    fn composite_device_attached(&mut self, composite_device: CompositeDeviceClient) {
        ...
        tokio::task::spawn(async move {
            let capabilities = match composite_device.get_capabilities().await {
                Ok(caps) => caps,
                Err(e) => {
                    log::error!("Failed to fetch composite device capabilities: {e:?}");
                    return;
                }
            };
            if let Err(e) = tx.send(capabilities).await {
                log::error!("Failed to send composite device capabilities: {e:?}");
            }
        });
    ...
    fn write_event(&mut self, event: NativeEvent) -> Result<(), InputError> {
        ...
        self.state.update(&self.capability_report, event.into());

        // Update the state version whenever an input event is received
        let state_version = self.state.state_version.to_primitive();
        self.state.state_version = Integer::from_primitive(state_version.wrapping_add(1));
        ...
        match capability {
            ...
            Capability::Gamepad(gamepad) => match gamepad {
                Gamepad::Button(button) => match button {
                    GamepadButton::South => Self::GAMEPAD_BUTTON_SOUTH,
                    GamepadButton::East => Self::GAMEPAD_BUTTON_EAST,
                    GamepadButton::North => Self::GAMEPAD_BUTTON_NORTH,
                    GamepadButton::West => Self::GAMEPAD_BUTTON_WEST,
                    GamepadButton::Start => Self::GAMEPAD_BUTTON_START,
                    GamepadButton::Select => Self::GAMEPAD_BUTTON_SELECT,
                    GamepadButton::Guide => Self::GAMEPAD_BUTTON_GUIDE,
                    GamepadButton::QuickAccess => Self::GAMEPAD_BUTTON_QUICK,
                    GamepadButton::QuickAccess2 => Self::GAMEPAD_BUTTON_QUICK2,
                    GamepadButton::Keyboard => Self::GAMEPAD_BUTTON_KEYBOARD,
                    GamepadButton::Screenshot => Self::GAMEPAD_BUTTON_SCREENSHOT,
                    GamepadButton::Mute => Self::GAMEPAD_BUTTON_MUTE,
                    GamepadButton::DPadUp => Self::GAMEPAD_BUTTON_DPAD_UP,
                    GamepadButton::DPadDown => Self::GAMEPAD_BUTTON_DPAD_DOWN,
                    GamepadButton::DPadLeft => Self::GAMEPAD_BUTTON_DPAD_LEFT,
                    GamepadButton::DPadRight => Self::GAMEPAD_BUTTON_DPAD_RIGHT,
                    GamepadButton::LeftBumper => Self::GAMEPAD_BUTTON_LEFT_BUMPER,
                    GamepadButton::LeftTop => Self::default(),
                    GamepadButton::LeftTrigger => Self::GAMEPAD_BUTTON_LEFT_TRIGGER,
                    GamepadButton::LeftPaddle1 => Self::GAMEPAD_BUTTON_LEFT_PADDLE1,
                    GamepadButton::LeftPaddle2 => Self::GAMEPAD_BUTTON_LEFT_PADDLE2,
                    GamepadButton::LeftPaddle3 => Self::GAMEPAD_BUTTON_LEFT_PADDLE3,
                    GamepadButton::LeftStick => Self::GAMEPAD_BUTTON_LEFT_STICK,
                    GamepadButton::LeftStickTouch => Self::GAMEPAD_BUTTON_LEFT_STICK_TOUCH,
                    GamepadButton::RightBumper => Self::GAMEPAD_BUTTON_RIGHT_BUMPER,
                    GamepadButton::RightTop => Self::default(),
                    GamepadButton::RightTrigger => Self::GAMEPAD_BUTTON_RIGHT_TRIGGER,
                    GamepadButton::RightPaddle1 => Self::GAMEPAD_BUTTON_RIGHT_PADDLE1,
                    GamepadButton::RightPaddle2 => Self::GAMEPAD_BUTTON_RIGHT_PADDLE2,
                    GamepadButton::RightPaddle3 => Self::GAMEPAD_BUTTON_RIGHT_PADDLE3,
                    GamepadButton::RightStick => Self::GAMEPAD_BUTTON_RIGHT_STICK,
                    GamepadButton::RightStickTouch => Self::GAMEPAD_BUTTON_RIGHT_STICK_TOUCH,
                },
                Gamepad::Axis(axis) => match axis {
                    GamepadAxis::LeftStick => Self::GAMEPAD_AXIS_LEFT_STICK,
                    GamepadAxis::RightStick => Self::GAMEPAD_AXIS_RIGHT_STICK,
                ...
                Gamepad::Trigger(trigger) => match trigger {
                    GamepadTrigger::LeftTrigger => Self::GAMEPAD_TRIGGER_LEFT,
                    GamepadTrigger::LeftTouchpadForce => Self::GAMEPAD_TRIGGER_LEFT_TOUCHPAD_FORCE,
                    GamepadTrigger::LeftStickForce => Self::GAMEPAD_TRIGGER_LEFT_STICK_FORCE,
                    GamepadTrigger::RightTrigger => Self::GAMEPAD_TRIGGER_RIGHT,
                    GamepadTrigger::RightTouchpadForce => {
                        Self::GAMEPAD_TRIGGER_RIGHT_TOUCHPAD_FORCE
                    }
                    GamepadTrigger::RightStickForce => Self::GAMEPAD_TRIGGER_RIGHT_STICK_FORCE,
                },
                Gamepad::Accelerometer => Self::GAMEPAD_ACCELEROMETER_CENTER,
                Gamepad::Gyro => Self::GAMEPAD_GYRO_CENTER,
            },
            Capability::Mouse(_) => Self::default(),
            Capability::Keyboard(_) => Self::default(),
            Capability::Touchpad(touchpad) => match touchpad {
                Touchpad::LeftPad(pad) => match pad {
                    Touch::Motion => Self::TOUCHPAD_LEFT_MOTION,
                    Touch::Button(_) => Self::TOUCHPAD_LEFT_BUTTON,
                },
                Touchpad::RightPad(pad) => match pad {
                    Touch::Motion => Self::TOUCHPAD_RIGHT_MOTION,
                    Touch::Button(_) => Self::TOUCHPAD_RIGHT_BUTTON,
                },
                Touchpad::CenterPad(pad) => match pad {
                    Touch::Motion => Self::TOUCHPAD_CENTER_MOTION,
                    Touch::Button(_) => Self::TOUCHPAD_CENTER_BUTTON,
                },
            },
            Capability::Touchscreen(_) => Self::default(),
        }
    ...
    fn from(value: Capability) -> Self {
        ...
        match value {
            ...
            Capability::Gamepad(gamepad) => match gamepad {
                ...
                Gamepad::Axis(_) => Self {
                    capability,
                    value_type: ValueType::UInt16Vector2,
                    offset: 0,
                },
                Gamepad::Trigger(_) => Self {
                    capability,
                    value_type: ValueType::UInt8,
                    offset: 0,
                },
                Gamepad::Accelerometer => Self {
                    capability,
                    value_type: ValueType::Int16Vector3,
                    offset: 0,
                },
                Gamepad::Gyro => Self {
                    capability,
