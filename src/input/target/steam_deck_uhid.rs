...
impl SteamDeckUhidDevice {
    ...
    fn handle_set_report(
        ...
        self.current_report = (*report_id).try_into()?;
        log::debug!("True Report ID: {:#04x}", (*report_id));
        let output_events = match self.current_report {
            ReportType::TriggerHapticCommand => {
                let buf = data.as_slice().try_into()?;

                let packed_haptic_report = PackedHapticReport::unpack(buf)?;
            ...
            ReportType::TriggerRumbleCommand => {
                let buf = data.as_slice().try_into()?;
                let packed_rumble_report = PackedRumbleReport::unpack(buf)?;
