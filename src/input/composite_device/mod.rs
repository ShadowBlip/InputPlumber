...
use sources::CompositeDeviceSources;
...
use super::{manager::ManagerCommand, target::client::TargetDeviceClient};
...
pub struct CompositeDevice {
    ...
    /// State of source devices attached to the composite device
    sources: CompositeDeviceSources,
...
impl CompositeDevice {
    pub fn new(
        ...
        let sources = CompositeDeviceSources::new(
            conn.clone(),
            dbus_path.clone(),
            tx.clone().into(),
            capability_map.as_ref(),
        );
        let targets = CompositeDeviceTargets::new(
            conn.clone(),
            dbus_path.clone(),
            tx.clone().into(),
            manager,
    ...
    pub async fn run(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        self.sources.run_devices().await?;
        ...
        'main: loop {
            ...
            for cmd in buffer.drain(..) {
                ...
                match cmd {
                    ...
                    CompositeCommand::ProcessOutputEvent(event) => {
                        if let Err(e) = self.sources.process_output_event(event).await {
                            log::error!("Failed to process output event: {:?}", e);
                        }
                    }
                    CompositeCommand::GetCapabilities(sender) => {
                        if let Err(e) = sender.send(self.sources.get_capabilities()).await {
                    ...
                    CompositeCommand::GetSourceDevicePaths(sender) => {
                        if let Err(e) = sender.send(self.sources.get_device_paths()).await {
            ...
            if devices_removed && !self.sources.has_source_devices() {
                if persist {
        ...
        self.sources.unhide_devices().await;

        // Send stop command to all source devices
        self.sources.stop().await;
    ...
    async fn process_event(
        ...
        if self.sources.is_blocked_device(device_id.as_str()) {
    ...
    async fn on_source_device_removed(&mut self, device: UdevDevice) -> Result<(), Box<dyn Error>> {
        self.sources.remove_device(device).await?;
    ...
    fn add_source_device(
        ...
        self.sources.add_device(device, &self.config)?;
