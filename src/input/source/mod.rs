use std::{collections::HashSet, error::Error, future::Future, str::FromStr, time::Duration};
...
impl<T: SourceInputDevice + SourceOutputDevice + Send> SourceDriver<T> {
    ...
    async fn process_events(&self, events: Vec<NativeEvent>) -> Result<(), Box<dyn Error>> {
        ...
        for event in events.into_iter() {
            ...
            self.composite_device
                .process_event(device_id.clone(), event)
                .await?;
        }
