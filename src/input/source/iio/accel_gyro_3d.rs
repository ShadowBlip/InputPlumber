    ...
    match event {
        iio_imu::event::Event::Accelerometer(data) => {
            ...
            let value = InputValue::Vector3 {
                x: Some(data.roll * ACCEL_CORRECTION),
                y: Some(data.pitch * ACCEL_CORRECTION),
                z: Some(data.yaw * ACCEL_CORRECTION),
            };
        ...
        iio_imu::event::Event::Gyro(data) => {
            ...
            let value = InputValue::Vector3 {
                x: Some(data.roll * GYRO_CORRECTION),
                y: Some(data.pitch * GYRO_CORRECTION),
                z: Some(data.yaw * GYRO_CORRECTION),
            };
            NativeEvent::new(cap, value)
        }
    }
