...
impl TryFrom<String> for ColorType {
    ...
    fn try_from(value: String) -> Result<Self, Self::Error> {
        match value.as_str() {
            ...
            _ => Err(LedRgbError::UnsupportedColor(value)),
        }
    }
}

/// RGB/multicolor LED source device implementation. Handles LEDs that use
/// the kernel `multi_intensity` sysfs interface, including both RGB LEDs
/// (packed u32 color) and multicolor LEDs (separate color channels).
pub struct LedRgb {
...
impl LedRgb {
    /// Create a new LedRgb source device with the given udev
    /// device information
    pub fn new(device_info: UdevDevice) -> Result<Self, Box<dyn Error + Send + Sync>> {
        let brightness_path = PathBuf::from(device_info.syspath().as_str()).join("brightness");
        if !brightness_path.exists() {
            return Err(Box::new(LedRgbError::Path(brightness_path)));
        }
        ...
        if !multi_intensity_path.exists() {
            return Err(Box::new(LedRgbError::Path(multi_intensity_path)));
        }
        ...
        log::debug!(
            "LED RGB Device setup complete. brightness: {}, max_brightness {}, color {:?},",
            current_brightness,
            max_brightness,
            current_color
        );
    ...
    fn write_color(&self, r: u8, g: u8, b: u8) -> Result<(), Box<dyn Error + Send + Sync>> {
        let contents = read_to_string(self.multi_intensity_path.as_path())
            .map_err(|err| Box::new(LedRgbError::Read(err)))?;
        let contents = (self
            .multi_index_map
            .iter()
        ...
        Ok(fs::write(self.multi_intensity_path.as_path(), contents)
            .map_err(|err| Box::new(LedRgbError::Write(err)))?)
    }
    ...
    #[allow(dead_code)]
    fn read_color(&self) -> Result<Vec<HashMap<ColorType, u8>>, LedRgbError> {
        read_color(
...
impl Debug for LedRgb {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("LedRgb").finish()
    }
}

impl SourceInputDevice for LedRgb {
...
impl SourceOutputDevice for LedRgb {
...
fn read_color(
    path: &Path,
    index: &[ColorType],
) -> Result<Vec<HashMap<ColorType, u8>>, LedRgbError> {
    let mut current_color: Vec<HashMap<ColorType, u8>> = Vec::new();

    let color_contents = read_to_string(path).map_err(LedRgbError::Read)?;
...
fn read_color_index(path: &Path) -> Result<Vec<ColorType>, LedRgbError> {
    let mi_contents = read_to_string(path).map_err(LedRgbError::Read)?;
