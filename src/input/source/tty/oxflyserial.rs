use crate::{
    drivers::{
        oxfly_tty::{
            ...
            STICK_MAX, STICK_MIN, TRIGG_MAX,
        },
        unified_gamepad::value,
    },
    input::{
...
impl OneXFlySerial {
    ...
    pub fn new(device_info: UdevDevice) -> Result<Self, Box<dyn Error + Send + Sync>> {
        log::debug!("OneXFly Serial Device setup complete.");
    ...
    fn translate_event(&mut self, event: event::Event) -> NativeEvent {
        match event {
            event::Event::GamepadButton(button) => match button {
                event::GamepadButtonEvent::None => {
                    NativeEvent::new(Capability::None, InputValue::None)
                }
            ...
            event::Event::Axis(axis) => match axis.clone() {
                AxisEvent::LStick(value) => NativeEvent::new(
                    Capability::Gamepad(Gamepad::Axis(GamepadAxis::LeftStick)),
                    normalize_axis_value(axis),
                ),
                AxisEvent::RStick(value) => NativeEvent::new(
                    Capability::Gamepad(Gamepad::Axis(GamepadAxis::RightStick)),
                    normalize_axis_value(axis),
                ),
                AxisEvent::TriggerL(value) => NativeEvent::new(
                    Capability::Gamepad(Gamepad::Trigger(GamepadTrigger::LeftTrigger)),
                    normalize_axis_value(axis),
                ),
                AxisEvent::TriggerR(value) => NativeEvent::new(
