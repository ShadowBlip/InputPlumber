...
use crate::{
    drivers::lego::{
        driver_dinput_attached::{self, Driver},
        event,
    },
    input::{
        capability::{
...
pub struct LegionControllerDA {
    driver: Driver,
}

impl LegionControllerDA {
...
impl SourceInputDevice for LegionControllerDA {
...
impl SourceOutputDevice for LegionControllerDA {}

impl Debug for LegionControllerDA {
...
fn normalize_axis_value(event: event::AxisEvent) -> InputValue {
    match event {
        event::AxisEvent::Touchpad(value) => {
            let max = driver_dinput_attached::PAD_X_MAX;
            let x = normalize_unsigned_value(value.x as f64, max);

            let max = driver_dinput_attached::PAD_Y_MAX;
        ...
        event::AxisEvent::LStick(value) => {
            let min = driver_dinput_attached::STICK_X_MIN;
            let max = driver_dinput_attached::STICK_X_MAX;
            let x = normalize_signed_value(value.x as f64, min, max);
            let x = Some(x);

            let min = driver_dinput_attached::STICK_Y_MAX; // uses inverted Y-axis
            let max = driver_dinput_attached::STICK_Y_MIN;
...
fn normalize_trigger_value(event: event::TriggerEvent) -> InputValue {
    match event {
        event::TriggerEvent::ATriggerL(value) => {
            let max = driver_dinput_attached::TRIGG_MAX;
        ...
        event::TriggerEvent::MouseWheel(value) => InputValue::None,
    }
}
