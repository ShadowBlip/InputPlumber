...
pub struct GpdTouchpad2023 {
    driver: TouchpadDriver2023,
}

impl GpdTouchpad2023 {
    pub fn new(device_info: UdevDevice) -> Result<Self, Box<dyn Error + Send + Sync>> {
        let driver = TouchpadDriver2023::new(device_info)?;
        Ok(Self { driver })
    }
}

impl SourceInputDevice for GpdTouchpad2023 {
...
impl SourceOutputDevice for GpdTouchpad2023 {}

impl Debug for GpdTouchpad2023 {
...
fn normalize_axis_value(event: event::TouchAxisEvent) -> InputValue {
    ...
    let max = TOUCHPAD_2023_X_MAX;
    let x = normalize_unsigned_value(x as f64, max);

    let max = TOUCHPAD_2023_Y_MAX;
