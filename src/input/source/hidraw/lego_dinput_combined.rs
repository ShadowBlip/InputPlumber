...
use crate::{
    drivers::lego::{
        driver_dinput_combined::{self, Driver},
        event,
    },
    input::{
        capability::{
...
impl SourceOutputDevice for LegionControllerDCombined {}

impl Debug for LegionControllerDCombined {
...
fn normalize_axis_value(event: event::AxisEvent) -> InputValue {
    match event {
        event::AxisEvent::Touchpad(value) => {
            let max = driver_dinput_combined::PAD_X_MAX;
            let x = normalize_unsigned_value(value.x as f64, max);

            let max = driver_dinput_combined::PAD_Y_MAX;
        ...
        event::AxisEvent::LStick(value) => {
            let min = driver_dinput_combined::STICK_X_MIN;
            let max = driver_dinput_combined::STICK_X_MAX;
            let x = normalize_signed_value(value.x as f64, min, max);
            let x = Some(x);

            let min = driver_dinput_combined::STICK_Y_MAX; // uses inverted Y-axis
            let max = driver_dinput_combined::STICK_Y_MIN;
...
fn normalize_trigger_value(event: event::TriggerEvent) -> InputValue {
    match event {
        event::TriggerEvent::ATriggerL(value) => {
            let max = driver_dinput_combined::TRIGG_MAX;
